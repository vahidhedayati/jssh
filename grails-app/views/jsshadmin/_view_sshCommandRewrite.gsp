...
<g:set var="entityName"
	value="${message(code: 'ssh.Server.command.rewrite.label', default: 'Jssh Command Rewrite')}" />
...
<a data-toggle="modal" href="#masterAdminContainer" class="btn btn-success"	onclick="javascript:addSshUserRewrite();"> 
	...
	<div id="list-jsshuser" class="content scaffold-list" role="main">
		...
		<div class="row">
			...
			<div class="col-md-2">
				<g:sortableColumn property="replacement"
					title="${message(code: 'jsshuser.replacement.label', default: 'replacement')}"
					params="[id:id, lookup:lookup, loadBootStrap:loadBootStrap, loadJQuery:loadJQuery, loadStyle:loadStyle]" />
			</div>
			...
			<div class="row ${(i % 2) == 0 ? 'even' : 'odd'}">
				<div class="col-md-2">
					<g:link controller="connectSsh" action="edit"
						id="${userInstance.id}" params="${[table: 'sshCommandRewrite'] }">
						${fieldValue(bean: userInstance, field: "command")}
					</g:link>
				</div>
					<div class="col-md-2">
					${fieldValue(bean: userInstance, field: "replacement")}
				</div>
