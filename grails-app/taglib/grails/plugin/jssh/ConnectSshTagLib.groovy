...
class ConnectSshTagLib {
	...
	def socketconnect =  { attrs, body ->
		...
		def model=[hideWhatsRunning:hideWhatsRunning,hideDiscoButton:hideDiscoButton,
			...
			hideConsoleMenu:hideConsoleMenu,hideSendBlock:hideSendBlock,divId:divId,
			wshostname:wshostname,hostname:hostname,port:port,password:password,
			username:username,userCommand:userCommand]
		if (template) {
